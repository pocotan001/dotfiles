...
function exportListContainsItem(exportList, name) {
    if (!exportList)
        return false;
...
exports.exportListContainsItem = exportListContainsItem;
//
// the 'magic numbers' which make this extension possible :-)
//
// a quick list of keywords we probably want to skip asap.
const commonKeywordList = ['window', 'dom', 'array', 'from', 'null', 'return', 'get', 'set', 'boolean', 'string', 'if', 'var', 'let', 'const', 'for', 'public', 'class', 'interface', 'new', 'import', 'as', 'private', 'while', 'case', 'switch', 'this', 'function', 'enum'];
...
const commonKeywordStartsWith = ['copy', 'id', 'ready', 'cancel', 'build', 'finish', 'merge', 'clamp', 'construct', 'native', 'clear', 'update', 'parse', 'sanitize', 'render', 'has', 'equal', 'dispose', 'create', 'as', 'is', 'init', 'process', 'get', 'set'];
// paths to ignore while looking through node_modules 
const commonIgnorePaths = ['esm', 'testing', 'test', 'facade', 'backends', 'es5', 'es2015', 'umd'];
// all library (node_modules) paths which should always be ignored
//
const commonIgnoreLibraryPaths = vscode.workspace.getConfiguration('genGetSet').get('ignoredLibraryPaths');
//
const ignoredLibraryList = vscode.workspace.getConfiguration('genGetSet').get('ignoredNodeLibraries');
//
const ignoredImportList = vscode.workspace.getConfiguration('genGetSet').get('ignoredImportList');
//
const ignoredDictionaryList = vscode.workspace.getConfiguration('genGetSet').get('ignoredDictionaryList');
        ...
        for (let i = 0; i < lineCount; i++) {
            ...
            if (matches) {
                ...
                if (_export !== null) {
                    ...
                    if (others.length > 0)
                        others.forEach(o => _export.exported.push(o));
                    const range = new vscode.Range(new vscode.Position(i, 0), new vscode.Position(i + 1, 0));
                    builder.replace(range, createImportLine(_export));
                    return;
                }
    ...
    for (let i = 0; i < file.lineCount; i++) {
        ...
        for (let j = 0; j < matches.length; j++) {
            ...
            if (matches[j].indexOf('\'') === -1 &&
                ...
                const _word = splitted.length > 0 ? splitted[0] : matches[j];
